...
2. KEEP ONE CUMULATIVE OFFSET PER SESSION. NEVER LET IT DECREASE.
...
onInterruptionConfirmed(() => {
  send({ type: 'Interrupt', playback_offset_ms: Math.round(ledger.offsetForConfirmedBuzz()) });
});
...
onInterruptionConfirmed(() => {
  send({ type: 'Interrupt', playback_offset_ms: playback.positionMs() });
});
