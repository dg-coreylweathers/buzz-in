...
**0. A note on the contract you are monitoring against.** The offset travels
as `playback_offset: {"type":"time_ms","value":N}`, measured from the start of
the session's audio, and each interrupt's offset must advance past the one
before it. Equal is rejected, not just smaller. That is the invariant the
signals below are watching.
